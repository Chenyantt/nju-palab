...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
  ...
  uint8_t bpp = dst->format->BitsPerPixel / 8;
  uint32_t src_pos, dst_pos, w, h;
  if (srcrect == NULL)
  {
    src_pos = 0;
    w = src->w;
  ...
  {
    src_pos = srcrect->x * bpp + srcrect->y * src->pitch;
    w = srcrect->w;
    h = srcrect->h;
  }
  if (dstrect == NULL)
  ...
  {
    dst_pos = dstrect->x * bpp + dstrect->y * dst->pitch;
  }
  for (int i = 0; i < h; ++i)
  {
    for (int j = 0; j < w * bpp; ++j)
    {
      dst->pixels[dst_pos + j] = src->pixels[src_pos + j];
    }
    dst_pos += dst->pitch;
    src_pos += src->pitch;
  }
